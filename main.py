# -*- coding: utf-8 -*-
"""
A library web app using Flask framework, python and sqlite3. 
...
def user_entries():
    ...
    return render_template("user_books.html", books=books, borrowedBooks=borrowedBooks)
...
#Create image along the username from en.gravatar.com 
...
def signup():
    if request.method=='POST':
        ...
        return render_template('user_books.html', users=users)
    else:
...
def loginadmin():
    error = None
    if request.method == "POST":
        if request.form["username"] != "admin" or request.form["password"] != 'pass123@' :
    ...
    return render_template("logadmin.html", error=error)
